...
void logSessionBooted(const char* playerName);
...
void logSessionBooted(const char* playerName) {
    ...
    String logEntry = String(timestamp) + " | BOOTED | " + String(playerName) + "\n";
    logFile.print(logEntry);
    logFile.close();
}
...
void cmdDownload(Player &p, const String &filename) {
    ...
    if (fname.equalsIgnoreCase("all")) {
        ...
        int fileCount = 0;
        int pos = 0;
        while ((pos = listResponse.indexOf("\"name\":", pos)) != -1) {
            fileCount++;
            pos += 7;
        }
        
        if (fileCount == 0) {
            p.client.println("Error: No files found in server response");
            return;
        }
        
        // Download each file with timing info using PERSISTENT CONNECTION
        ...
        if (!wifiClient.connect("www.storyboardacs.com", 80)) {
            p.client.println("Error: Could not connect to server");
            return;
        }
        ...
        while (true) {
            ...
            String remoteFilename = listResponse.substring(nameStartPos + 1, nameEndPos);
            ...
            int sizeStartPos = sizePos + 7;
            while (sizeStartPos < listResponse.length() && !isdigit(listResponse[sizeStartPos])) {
                sizeStartPos++;
            }
            int sizeEndPos = sizeStartPos;
            while (sizeEndPos < listResponse.length() && isdigit(listResponse[sizeEndPos])) {
                sizeEndPos++;
            }
            int fileSize = listResponse.substring(sizeStartPos, sizeEndPos).toInt();
            
            // Display progress
            float sizeKB = fileSize / 1024.0;
            p.client.print("[" + String(currentFile) + "/" + String(fileCount) + "] " + 
                          remoteFilename + "... " + String(sizeKB, 1) + " KB... ");
            
            unsigned long fileStartTime = millis();
            
            // Send HTTP GET request on persistent connection with Keep-Alive
            wifiClient.print("GET /download.php?file=" + remoteFilename + " HTTP/1.1\r\n");
            wifiClient.print("Host: www.storyboardacs.com\r\n");
            wifiClient.print("Connection: keep-alive\r\n");
            wifiClient.print("\r\n");
            wifiClient.flush();
            
            // Skip HTTP headers by counting newlines until blank line
            int newlineCount = 0;
            unsigned long headerTimeout = millis();
            while (newlineCount < 2 && millis() - headerTimeout < 5000) {
            ...
            // Now read exactly fileSize bytes of file data
            ...
            const unsigned long READ_TIMEOUT = 15000;
            
            while (bytesWritten < fileSize && millis() - readTimeout < READ_TIMEOUT) {
                if (wifiClient.available()) {
                    int toRead = (fileSize - bytesWritten < 512) ? (fileSize - bytesWritten) : 512;
                    int bytesRead = wifiClient.readBytes(buffer, toRead);
                    if (bytesRead > 0) {
            ...
            if (bytesWritten == fileSize) {
                unsigned long fileElapsed = millis() - fileStartTime;
                float fileSpeed = (fileSize / 1024.0) / (fileElapsed / 1000.0);
                p.client.println("[" + String(fileElapsed) + "ms, " + String(fileSpeed, 1) + " KB/s]");
                filesDownloaded++;
                totalBytes += fileSize;
            } else {
                p.client.println("(incomplete: " + String(bytesWritten) + "/" + String(fileSize) + ")");
                // Don't stop the persistent connection, just log the error and continue
            }
...
void cmdWizHelp(Player &p) {
    ...
    p.client.println("debug sessions          - Show last 40 session log records");
    ...
    switch (st.stage) {
        ...
        // -------------------------
        {
            ...
            if (p.IsWizard) {
                ...
                // Program size (flash) - APP0 partition: 0x10000-0x2D0000 = 2,883,584 bytes (2816 KB)
                uint32_t sketchSize = ESP.getSketchSize();
                uint32_t appPartitionSize = 2883584;  // 0x2C0000 = 2,883,584 bytes from no_ota.csv (2816 KB)
                ...
                // LittleFS space - LITTLEFS partition: 0x2D0000-0x410000 = 1,310,720 bytes (1280 KB)
                size_t fsUsed = LittleFS.usedBytes();
                uint32_t fsTotal = 1310720;  // 0x140000 = 1,310,720 bytes from no_ota.csv (1280 KB)
...
if (cmd == "debug") {
    ...
    if (a == "sessions") {
        ...
        // Display the file contents in REVERSE order (most recent first)
        debugPrint(p, "=== SESSION LOG (Last 40 Records - NEWEST FIRST) ===");
        ...
        // Print in reverse order (last line first), up to 40 records
        int linesToShow = lines.size();
        if (linesToShow > 40) {
            linesToShow = 40;
        }
        
        for (int i = lines.size() - 1; i >= (int)(lines.size() - linesToShow); i--) {
            if (i >= 0) {
                debugPrint(p, lines[i]);
            }
        }
...
for (auto &npc : npcInstances) {
    ...
    // Jokes are pre-loaded from jokes.txt file (no API calls during gameplay)
    {
        ...
        if (playersInRoom > 0) {
            ...
            if (!innKeeperJokes.active) {
                // First time players entered: schedule first joke for 15-20 seconds from now
                innKeeperJokes.nextJokeTime = now + random(15000, 20001);
                Serial.println("[JOKE] Players entered room, first joke scheduled for 15-20 seconds");
            }
            ...
            if (now >= innKeeperJokes.nextJokeTime) {
                Serial.println("[JOKE] ==== JOKE DELIVERY START ====");
                try {
                    // Get random joke from jokes.txt
                    Serial.println("[JOKE] Calling getRandomJoke()...");
                    String jokeText = getRandomJoke();
                    Serial.printf("[JOKE] Got joke text (%d chars): %.50s...\n", jokeText.length(), jokeText.c_str());
                    
                    // CRITICAL: Clean up any embedded newlines/extra spaces/garbage
                    Serial.println("[JOKE] Starting cleanup phase...");
                    jokeText.replace("\n", " ");
                    jokeText.replace("\r", " ");
                    jokeText.replace("\t", " ");
                    jokeText.replace("\0", "");  // Remove null terminators
                    Serial.println("[JOKE] Escape sequences cleaned");
                    
                    // Collapse multiple spaces to single space (with iteration limit to prevent infinite loop)
                    int collapseAttempts = 0;
                    Serial.println("[JOKE] Starting space collapse...");
                    while (jokeText.indexOf("  ") != -1 && collapseAttempts < 50) {
                        jokeText.replace("  ", " ");
                        collapseAttempts++;
                    }
                    Serial.printf("[JOKE] Space collapse complete (%d attempts)\n", collapseAttempts);
                    if (collapseAttempts >= 50) {
                        Serial.println("[JOKE] WARNING: Space collapse hit iteration limit in game loop");
                    }
                    jokeText.trim();
                    Serial.println("[JOKE] Trimmed whitespace");
                    
                    // Remove any trailing junk after last printable character
                    Serial.println("[JOKE] Removing trailing junk...");
                    ...
                    Serial.println("[JOKE] Calling announceToRoom()...");
                    announceToRoom(JOKE_ROOM_X, JOKE_ROOM_Y, JOKE_ROOM_Z, jokeMsg, -1);
                    Serial.println("[JOKE] announceToRoom() complete");
                    
                    // Schedule next joke (15-20 seconds from now)
                    innKeeperJokes.nextJokeTime = now + random(15000, 20001);
                    Serial.println("[JOKE] ==== JOKE DELIVERY SUCCESS ====");
                } catch (const std::exception &e) {
                    Serial.printf("[JOKE] ERROR: Exception caught: %s\n", e.what());
                    innKeeperJokes.nextJokeTime = now + 60000;  // Retry in 60 seconds
                } catch (...) {
                    Serial.println("[JOKE] ERROR: Unknown exception in joke delivery!");
                    innKeeperJokes.nextJokeTime = now + 60000;  // Retry in 60 seconds
                }
            }
        } else {
